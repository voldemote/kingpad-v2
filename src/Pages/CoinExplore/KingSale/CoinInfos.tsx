      ...
      <CoinInfosBox>
        ...
        <CoinInfo
          ...
          value={
            <CopyToClipboard>
              {({ copy }) => (
                <CoinInfoValue onClick={() => copy(data.token_address)}>{data.token_address}</CoinInfoValue>
              )}
            </CopyToClipboard>
          }
          cursor="pointer"
        />
        <CoinInfo name="Total Supply" value={commaSeparators(data.total_supply)} />
        <CoinInfo name="Liquidity Percent" value={`${data.liquidity_percentage}%`} />
        <CoinInfo name="Presale Start Time" value={`${data.presale_start} (UTC)`} />
        <CoinInfo name="Presale End Time" value={`${data.presale_end} (UTC)`} />
        <CoinInfo name="Presale Rate" value={`${data.presale_rate} ${data.symbol} per ${data.currency}`} />
        <CoinInfo name="Listing Rate" value={`${data.listing_rate} ${data.symbol} per ${data.currency}`} />
      </CoinInfosBox>
      <ContributionBox>
        <VestingCard>
        ...
        <DistributionBox>
          ...
          <ChartContainer>
            <DistributionChart projectId={projectId} />
          </ChartContainer>
          <TokenChartInfo projectId={projectId} />
        </DistributionBox>
      </ContributionBox>
    </CoinInfosContainer>
  );
...
const CoinInfo = (props: CoinInfoProps) => {
  return (
    <CoinInfoWrapper sx={{ cursor: props.cursor !== undefined ? props.cursor : 'auto' }}>
